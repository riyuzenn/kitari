                ...
                <div className="grid grid-cols-[repeat(auto-fill,minmax(47px,1fr))] gap-2 overflow-y-auto mt-1 mb-2">
                    {Array(total)
                        ...
                        .map((v, index) => {
                            ...
                            return (
                                <button
                                    onClick={() => router.push(`/stream/${id}/ep/${i}`)}
                                    disabled={router.asPath == `/stream/${id}/ep/${i}` ? true : false}
                                    className={`flex border border-daccent_1 ${
                                        router.asPath == `/stream/${id}/ep/${i}`
                                            ? "bg-[#937193] text-daccent_2 border-[#000]"
                                            : "bg-daccent_1 hover:bg-daccent_2 "
